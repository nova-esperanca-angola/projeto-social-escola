...
<div id="modal-apadrinhar" role="dialog" aria-modal="true" aria-labelledby="modal-titulo" aria-describedby="cota-desc" class="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm hidden transition-opacity">
  <div class="bg-surface-container-lowest w-full max-w-lg rounded-t-3xl sm:rounded-2xl p-6 shadow-2xl border border-outline-variant/30 space-y-4 max-h-[90vh] overflow-y-auto">
    ...
    <div class="flex justify-between items-center border-b border-outline-variant/20 pb-3">
      ...
      <button onclick="fecharModalApadrinhamento()" aria-label="Fechar janela de apadrinhamento" class="min-h-[48px] min-w-[48px] flex items-center justify-center text-on-surface-variant hover:text-on-background rounded-full hover:bg-surface-container transition-colors">
